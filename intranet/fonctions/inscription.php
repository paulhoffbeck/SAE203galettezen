...
function inscriptionFormulaire(){
    if(isset($_POST['register'])){
        $result = register();
        if($result === true){
            echo "<div class=\"alert alert-success\"><b>Inscription !</b> Vous avez reçu un email de confirmation pour activer votre compte.</div>";
        }else{
            echo "<div class=\"alert alert-warning\"><b>Erreur :</b> $result</div>";
        }
    }
    return '
    <br>
    ...
    if(isset($_POST["prenom"]) && isset($_POST["nom"]) && isset($_POST["email"]) && isset($_POST["password"]) && isset($_POST["cpassword"])){
        ...
        if ($domaine != "galetezen.com") {
            ...
            echo "<script>emailjs.send(\"service_29z78s7\", \"template_kyqv2ik\", {
                ...
                uid: \"$uid\",
                destinataire: \"teohubert00@gmail.com\"
            })
            .then(() => {
